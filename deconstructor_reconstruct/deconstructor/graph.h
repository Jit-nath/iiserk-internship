...
atom findAtom(const vector<atom>& molecular_structure, int resID) {
  ...
  // Return default atom if not found (should handle this error properly)
  return atom();
}

void dfs(int current, int parent, const map<int, vector<int>>& adj_list,
...
vector<node> createGraph(map<int, vector<int>> adjacency_list,
                         vector<atom> molecular_structure, atom root) {
  vector<node> nodes;
...
void printGraph(const vector<node>& graph) {
  cout << left << setw(8) << "Parent"
       << setw(8) << "Child"
       << setw(15) << "Bond Length"
       << setw(12) << "Bond Angle" << endl;
  cout << left << setw(8) << "------"
       << setw(8) << "-----"
       << setw(15) << "-----------"
       << setw(12) << "----------" << endl;
  for (const auto& n : graph) {
    cout << left << setw(8) << n.resID
         << setw(8) << n.child_resID
         << setw(15) << fixed << setprecision(4) << n.bond_length
         << setw(12) << fixed << setprecision(2) << n.bond_angle << endl;
  }
}